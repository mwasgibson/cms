...
http.interceptors.response.use(
  ...
  (err) => {
    if (err.response?.status === 401) {
      // Do not redirect auth requests themselves. In particular, /auth/profile
      // is intentionally allowed to return 401 while AuthProvider is checking
      // whether a session exists on the login page. Redirecting that request
      // back to /login causes an infinite 401 -> /login -> 401 loop.
